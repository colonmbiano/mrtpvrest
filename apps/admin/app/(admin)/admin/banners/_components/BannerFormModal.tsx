...
import { X, Image as ImageIcon, Upload, CalendarDays, Clock } from "lucide-react";
import { Field, Input, Select, Toggle, Button } from "@/components/ds";
...
export function BannerFormModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4" style={{ background: "rgba(11,18,32,.55)", backdropFilter: "blur(2px)" }}>
      <div
        role="dialog"
        aria-modal="true"
        aria-label={isEdit ? "Editar banner" : "Nuevo banner"}
        className="ds-enter my-4 w-full max-w-lg overflow-hidden rounded-ds-xl"
        style={{ background: "var(--surf-1)", border: "1px solid var(--bd-1)", boxShadow: "var(--shadow-lg)" }}
      >
        <div className="flex items-center justify-between border-b px-6 py-4" style={{ borderColor: "var(--bd-1)" }}>
          <h2 className="font-display text-xl font-extrabold text-tx-hi">{isEdit ? "Editar banner" : "Nuevo banner"}</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Cerrar"
        ...
        >
          ...
          <div className="flex gap-3">
            ...
            <Button type="submit" loading={saving} disabled={saving} full>
