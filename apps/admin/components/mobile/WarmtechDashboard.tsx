...
export default function WarmtechDashboard() {
  ...
  if (loading && !stats) {
    return <div className="grid grid-cols-2 gap-3 px-[18px] py-4">{Array.from({ length: 6 }).map((_, index) => <div key={index} className="h-28 animate-pulse rounded-[18px] bg-surf-2" />)}</div>;
  }
  ...
  return (
    <div className="warmtech-enter px-[18px] pb-6 pt-3">
      <div className="mb-3 flex rounded-[13px] p-1" style={{ background: "var(--surf-2)", border: "1px solid var(--bd-1)" }}>
      ...
      <section className="warmtech-card overflow-hidden p-3.5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="font-mono text-[10px] uppercase tracking-[.12em] text-tx-mut">Ventas · {periodLabels[period]}</div>
            <div className="mt-1.5 font-display text-[34px] font-extrabold leading-none tracking-[-.035em] text-tx-hi">{money(stats?.sales.value || 0)}</div>
          </div>
      ...
      <section className="mt-3 overflow-hidden rounded-[18px] p-3.5 text-white" style={{ background: "linear-gradient(135deg,#7d351f,#c95a2f)", boxShadow: "0 8px 22px var(--iris-glow)" }}>
      ...
      <div className="mb-3 mt-6 flex items-center justify-between">
        <h2 className="font-display text-base font-extrabold text-tx-hi">Top platillos</h2>
      ...
      <section className="warmtech-card overflow-hidden px-4">
      ...
      <h2 className="mb-3 mt-6 font-display text-base font-extrabold text-tx-hi">Acciones rápidas</h2>
      <div className="grid grid-cols-2 gap-3">
        ...
        <Link href="/admin/menu" className="warmtech-card col-span-2 flex min-h-[58px] items-center justify-center gap-2 p-3 text-xs font-bold text-primary"><PackagePlus size={17} /> Administrar platillos</Link>
      </div>
    </div>
  );
}
