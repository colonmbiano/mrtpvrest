...
export default function ComboBuilder({ itemId, products }: { itemId: string; products: ProductRef[] }) {
  ...
  async function addComponent() {
    ...
    } catch (e: any) { alert(e?.response?.data?.error || "Error al crear componente"); }
  }
  async function deleteComponent(id: string) {
    if (!confirm("¿Eliminar este componente y sus opciones?")) return;
  ...
  async function addOption(componentId: string) {
    ...
    try {
      ...
      await fetchComponents();
    } catch (e: any) { alert(e?.response?.data?.error || "Error al agregar opción"); }
  }
  async function deleteOption(optionId: string) {
    try { await api.delete(`/api/menu/combo-options/${optionId}`); await fetchComponents(); }
    catch (e: any) { alert(e?.response?.data?.error || "Error al eliminar opción"); }
  }

  if (loading) return <p className="text-xs px-1" style={{ color: "var(--muted)" }}>Cargando combo…</p>;
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {components.map((c) => {
        ...
        return (
          <div key={c.id} className="rounded-xl border p-3 flex flex-col gap-2" style={{ background: "var(--surf2)", borderColor: "var(--border)" }}>
            <div className="flex items-start justify-between gap-2">
              <div className="flex flex-1 flex-col gap-1.5">
                ...
                />
                <label className="flex items-center gap-1.5 text-[11px] font-bold" style={{ color: "var(--muted)" }}>
                  ...
                  <input
                    ...
                    className="w-12 bg-transparent text-center outline-none" style={{ color: "var(--text)" }}
                  />
                </label>
              </div>
              <button type="button" onClick={() => deleteComponent(c.id)} className="px-2 py-1 rounded-lg text-[11px] font-bold" style={{ background: "var(--surf)", border: "1px solid var(--border)", color: "#ff5c35" }}>
                Borrar
              </button>
            </div>

            <div className="flex flex-col gap-1">
              {c.options.length === 0 && <p className="text-[11px] px-1" style={{ color: "var(--muted)" }}>Sin opciones aún.</p>}
              {c.options.map((o) => (
                <div key={o.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg" style={{ background: "var(--surf)", border: "1px solid var(--border)" }}>
                  <span className="flex-1 truncate text-sm font-bold" style={{ color: "var(--text)" }}>{o.optionMenuItem?.name || o.optionMenuItemId}</span>
                  <span className="text-xs font-mono" style={{ color: "var(--muted)" }}>{o.priceDelta > 0 ? `+$${o.priceDelta}` : "—"}</span>
                  <button type="button" onClick={() => deleteOption(o.id)} className="w-6 h-6 rounded-md text-xs font-black" style={{ background: "var(--surf2)", color: "#ff5c35" }} aria-label="Eliminar opción">✕</button>
                </div>
              ))}
            </div>
            ...
            <div className="flex gap-1.5">
              ...
              <input
                ...
                style={{ background: "var(--surf)", border: "1px solid var(--border)", color: "var(--text)" }}
              />
              <button type="button" onClick={() => addOption(c.id)} disabled={!optForm.optionMenuItemId} className="px-3 py-1.5 rounded-lg text-xs font-black" style={{ background: "var(--gold)", color: "#000", opacity: optForm.optionMenuItemId ? 1 : 0.5 }}>+</button>
            </div>
          </div>
        );
      })}
      ...
      <div className="flex gap-1.5">
        <input
          ...
          style={{ background: "var(--surf)", border: "1px solid var(--border)", color: "var(--text)" }}
        />
        <button type="button" onClick={addComponent} disabled={!newName.trim()} className="px-3 py-1.5 rounded-lg text-xs font-black" style={{ background: "var(--gold)", color: "#000", opacity: newName.trim() ? 1 : 0.5 }}>
          + Componente
        </button>
      </div>
    </div>
