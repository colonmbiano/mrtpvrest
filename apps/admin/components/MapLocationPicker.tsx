...
// Leaflet + OpenStreetMap por CDN (mismo patrón que app/(admin)/admin/rastreo):
// gratis, sin cuenta ni token. El admin mueve el pin o toca el mapa para fijar
// el origen de la tienda; también puede usar el GPS del dispositivo.
//
// Controlado: recibe `value` y emite `onChange({lat,lng})`.
...
export function MapLocationPicker({
  ...
  return (
    <div style={{ position: "relative", width: "100%", height, borderRadius: 16, overflow: "hidden", border: "1px solid var(--bd-1)" }}>
      <div ref={containerRef} style={{ width: "100%", height: "100%", background: "var(--surf-2)" }} />
      ...
      {ready && !value && (
        <div
          style={{
            ...
            background: "var(--surf-1)", border: "1px solid var(--bd-1)",
            fontSize: 12, fontWeight: 600, color: "var(--tx-mut)",
          }}
        >
          Toca el mapa o arrastra el pin para marcar el origen de la tienda
        </div>
      )}
    </div>
  );
