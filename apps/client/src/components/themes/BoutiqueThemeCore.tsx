...
import {
  ...
  Instagram, Facebook, Music2, Banknote, CreditCard, Building2, Mail, Lock, Navigation, Gift,
} from 'lucide-react';
...
import tokkiCampaignHero from '../../../public/tokki/tokki-hero-banner-v1.png';
import {
  getAuth, clearAuth, loginCustomer, registerCustomer, type AuthState,
} from '../../lib/customerAuth';

// ══════════════════════════════════════════════════════════════════════════════
//  MOTOR VISUAL COMPARTIDO PARA MOCHI Y BEVERAGE
//  Mantiene en un solo sitio el flujo estable de catálogo, carrito, checkout y
//  autenticación. Cada wrapper activa su identidad visual de forma explícita.
...
// Migración visual de Tokki: su portada anterior era una captura completa del
// storefront. La sustituimos por el arte de campaña nuevo, pero cualquier URL
// distinta que se guarde después desde Administración seguirá teniendo prioridad.
const TOKKI_LEGACY_HERO = 'https://res.cloudinary.com/dqvgidive/image/upload/v1782269558/master-burgers/global/nzvwtifv0nqrslfmpkgc.png';
const TOKKI_CAMPAIGN_HERO = tokkiCampaignHero.src;
...
export type BoutiqueThemeProps = {
  data: { info: Info; menu: { categories: any[] }; locations: any[] };
};

type OrderMode = 'DELIVERY' | 'TAKEOUT';

export function BoutiqueTheme({ data, variant = 'mochi' }: BoutiqueThemeProps & { variant?: 'mochi' | 'beverage' }) {
  ...
  const beverageTheme = variant === 'beverage';
  ...
  return (
    ...
    <div className="min-h-screen relative" style={{ color: INK, fontFamily: BODY, background: beverageTheme ? '#F8F5FB' : `radial-gradient(1100px 520px at 50% -8%, #FBE6F5 0%, transparent 55%), radial-gradient(900px 500px at 92% 8%, #E4E0FF 0%, transparent 55%), linear-gradient(180deg, #F4EDFD 0%, #EFE9FB 100%)` }}>
      {!beverageTheme && <Decor />}
        ...
        {q ? (
          <main className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-5">
              {results.map((p: any) => <ProductCard key={p.id} p={p} accent={ACCENT} slug={slug} onOpen={() => pick(p)} beverageTheme={beverageTheme} fallbackLogo={info.logo} />)}
            </div>
          </main>
        ) : (
          <>
            <Hero info={info} accent={ACCENT} onOrder={() => scrollTo('mk-menu')} onMenu={() => scrollTo('mk-menu')} beverageTheme={beverageTheme} heroProduct={allItems[0]} />
            ...
            <CategoryNav categories={categories} activeCat={activeCat} accent={ACCENT} onPick={scrollToCat} beverageTheme={beverageTheme} />
            ...
            <div className="max-w-7xl mx-auto px-4 mt-8 lg:grid lg:grid-cols-[minmax(0,1fr)_338px] lg:gap-7 lg:items-start">
              <div className="min-w-0">
                ...
                {favorites.length > 0 && (
                  <section className="mb-12">
                    <SectionHead title="Favoritos de todos" heart accent={ACCENT} beverageTheme={beverageTheme} />
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-5">
                      {favorites.map((p: any) => <ProductCard key={`fav-${p.id}`} p={p} accent={ACCENT} slug={slug} onOpen={() => pick(p)} beverageTheme={beverageTheme} fallbackLogo={info.logo} />)}
                    </div>
                  </section>
                )}
                ...
                <div id="mk-menu">
                  {categories.map((category: any) => (
                    <section key={category.id} id={`mk-cat-${category.id}`} data-cat={category.id} className="mb-12 scroll-mt-[188px] md:scroll-mt-[150px]">
                      <SectionHead title={category.name} icon={category.imageUrl || beverageTheme ? undefined : categoryIcon(category.name)} image={category.imageUrl} beverageTheme={beverageTheme} />
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-5">
                        {(category.items || []).map((p: any) => <ProductCard key={p.id} p={p} accent={ACCENT} slug={slug} onOpen={() => pick(p)} beverageTheme={beverageTheme} fallbackLogo={info.logo} />)}
                      </div>
                    </section>
                  ))}
                </div>
              ...
              <aside className="hidden lg:block sticky top-[150px]">
                <DesktopCart accent={ACCENT} minOrder={minOrder} allItems={allItems}
                  ...
                  waNumber={waNumber} suggestions={favorites.length ? favorites : allItems} onAddSuggestion={pick} beverageTheme={beverageTheme} logo={info.logo} />
              </aside>
            </div>
      ...
      {quantity > 0 && !cartOpen && (
        <div className="lg:hidden fixed bottom-[calc(0.75rem+env(safe-area-inset-bottom))] left-3 right-3 z-40">
          <button onClick={() => setCartOpen(true)}
            ...
            <span className="flex items-center gap-1.5 px-5 py-3 rounded-[16px] text-white font-extrabold text-[14px]" style={{ background: beverageTheme ? ACCENT : PINK, boxShadow: `0 10px 22px ${beverageTheme ? ACCENT : PINK}55` }}>Ver carrito <ChevronRight className="w-4 h-4" /></span>
          </button>
        </div>
      )}
      ...
      {cartOpen && (
        <MobileCart accent={ACCENT} onClose={() => setCartOpen(false)} onCheckout={goCheckout}
          onWhatsApp={orderByWhatsApp} waNumber={waNumber} allItems={allItems} minOrder={minOrder} beverageTheme={beverageTheme} logo={info.logo} />
      )}
      ...
      {modalProduct && <ProductModal product={modalProduct} accent={ACCENT} variant={beverageTheme ? "beverage" : "light"} onClose={() => setModalProduct(null)} />}
...
//  HEADER
// ══════════════════════════════════════════════════════════════════════════════
function Header({ info, accent, waNumber, quantity, total, query, setQuery, orderMode, setOrderMode, auth, onLogin, onMyOrders, onSignOut, onCart, onWhatsApp, beverageTheme }: any) {
  const fmt = useMoney();
  const { t } = useLang();
  const tokkiIdentity = /^tok+i\s+bob+a$/i.test((info.name || '').trim()) || info.slug === 'tokki-bobba';
  return (
    <header className="sticky top-0 z-40 backdrop-blur-xl" style={{ background: 'rgba(255,253,248,0.82)', borderBottom: `1px solid ${CARD_BD}` }}>
      <div className="max-w-7xl mx-auto px-4">
        <div className="h-[64px] flex items-center gap-3">
          ...
          {beverageTheme ? (
            <div className="min-w-0 leading-none">
              {tokkiIdentity ? (
                <h1 className="whitespace-nowrap text-[19px] sm:text-[22px]" style={{ fontFamily: DISP, fontWeight: 900 }}><span style={{ color: '#8B5FC7' }}>TOKKI</span> <span style={{ color: '#78A83F' }}>BOBBA</span></h1>
              ) : (
                <h1 className="max-w-[150px] truncate text-[19px] sm:max-w-[240px] sm:text-[22px]" style={{ color: accent, fontFamily: DISP, fontWeight: 900 }}>{info.name?.trim() || 'Mi cafetería'}</h1>
              )}
              <p className="mt-1 hidden text-[8px] font-black uppercase tracking-[0.18em] sm:block" style={{ color: INK2 }}>{tokkiIdentity ? 'Bubble tea · buenos días' : 'Café · bebidas · buenos momentos'}</p>
            </div>
          ...
          {info.isOpen !== false && (
            <span className="hidden sm:flex items-center gap-1.5 shrink-0 px-2.5 h-7 rounded-full text-[11px] font-bold" style={{ background: '#16a34a14', color: '#15803d', border: '1px solid #16a34a40' }}>
        ...
        <div className="md:hidden pb-3 flex min-w-0 items-center gap-2 overflow-hidden">
          <DeliveryToggle orderMode={orderMode} setOrderMode={setOrderMode} accent={accent} />
          <div className="flex h-10 min-w-0 flex-1 items-center gap-2 overflow-hidden rounded-full px-3.5" style={{ background: '#fff', border: `1.5px solid ${CARD_BD}` }}>
            <Search className="w-4 h-4 shrink-0" style={{ color: FAINT }} />
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder={t('search')} className="w-full min-w-0 flex-1 bg-transparent text-sm outline-none" style={{ color: INK }} />
            {query && <button onClick={() => setQuery('')} aria-label="Limpiar"><X className="w-4 h-4" style={{ color: FAINT }} /></button>}
          </div>
        </div>
      </div>
...
//  HERO
// ══════════════════════════════════════════════════════════════════════════════
function Hero({ info, accent, onOrder, onMenu, beverageTheme, heroProduct }: { info: Info; accent: string; onOrder: () => void; onMenu: () => void; beverageTheme?: boolean; heroProduct?: any }) {
  const heroImg = info.heroImageUrl?.trim();
  if (beverageTheme) {
    const tokkiIdentity = /^tok+i\s+bob+a$/i.test((info.name || '').trim()) || info.slug === 'tokki-bobba';
    // La portada configurada en Administración tiene prioridad. Solo migramos
    // la captura legacy conocida de Tokki al nuevo arte; futuros cambios se
    // muestran automáticamente. El slot estable evita que cualquier imagen
    // panorámica vuelva a empujar el menú fuera del primer vistazo.
    const campaignHero = tokkiIdentity && heroImg === TOKKI_LEGACY_HERO ? TOKKI_CAMPAIGN_HERO : heroImg;
    const visual = campaignHero || heroProduct?.imageUrl || info.logo;
    const steps = ['Tamaño', 'Hielo', 'Base', 'Sabor', 'Topping', 'Final'];
    return (
      <section className="max-w-7xl mx-auto px-4 pt-4">
        <div className={`relative overflow-hidden rounded-[28px] border p-5 sm:p-7 lg:grid ${campaignHero ? 'lg:grid-cols-[minmax(0,1fr)_minmax(300px,420px)]' : 'lg:grid-cols-[minmax(0,1fr)_290px]'} lg:items-center lg:gap-8`} style={{ background: '#FFFDFB', borderColor: '#E8DDF5', boxShadow: '0 18px 44px rgba(83,55,119,.10)' }}>
          <div className="relative z-10">
            <span className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[10px] font-black uppercase tracking-[.16em]" style={{ background: '#EFE5FA', color: '#744AAA' }}><Sparkles className="h-3.5 w-3.5" /> {tokkiIdentity ? 'Crea tu Toki' : 'Crea tu bebida'}</span>
            <h2 className="mt-3 text-[34px] leading-[.95] sm:text-[48px]" style={{ color: INK, fontFamily: DISP, fontWeight: 900 }}>Tu bebida,<br /><span style={{ color: accent }}>tus reglas.</span></h2>
            <p className="mt-3 max-w-xl text-[13px] font-semibold sm:text-[15px]" style={{ color: INK2 }}>{tokkiIdentity ? 'Elige tu compañero y personalízalo paso a paso.' : 'Elige tu favorita y personalízala paso a paso.'} Más de mil combinaciones para encontrar la bebida ideal.</p>
            <div className="mt-4 grid grid-cols-3 gap-2 sm:flex sm:overflow-x-auto sm:pb-1 no-scrollbar" aria-label="Seis pasos para crear tu bebida">
              {steps.map((step, index) => <span key={step} className="min-w-0 rounded-xl border bg-white px-2 py-2 text-[9px] font-extrabold sm:shrink-0 sm:px-2.5 sm:text-[10px]" style={{ borderColor: '#E8DDF5', color: INK }}><b className="mr-1 inline-flex h-5 w-5 items-center justify-center rounded-full text-white sm:mr-1.5" style={{ background: index === 0 ? accent : '#B59BD6' }}>{index + 1}</b>{step}</span>)}
            </div>
            <button onClick={onOrder} className="mt-5 inline-flex min-h-12 items-center gap-2 rounded-2xl px-6 text-sm font-black text-white active:scale-95" style={{ background: accent, boxShadow: `0 12px 26px ${accent}45` }}>Empezar mi bebida <ChevronRight className="h-5 w-5" /></button>
          </div>
          {visual && <button onClick={onOrder} aria-label={campaignHero ? 'Ver promoción destacada' : 'Ver bebidas'} className={campaignHero ? 'mt-5 block h-36 w-full overflow-hidden rounded-2xl sm:h-52 lg:relative lg:mt-0 lg:h-64' : 'hidden sm:absolute sm:bottom-2 sm:right-5 sm:block sm:h-56 sm:w-56 lg:relative lg:bottom-auto lg:right-auto lg:h-64 lg:w-full'} style={campaignHero ? { background: '#F7F1FD', border: '1px solid #E8DDF5' } : undefined}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={campaignHero ? heroSrc(visual) : cldImage(visual, { width: 620 })} alt={campaignHero ? `Portada de ${info.name}` : (heroProduct?.name || info.name)} className={`h-full w-full object-contain ${campaignHero ? '' : 'drop-shadow-[0_18px_24px_rgba(83,55,119,.18)]'}`} />
          </button>}
        </div>
      </section>
    );
...
// ══════════════════════════════════════════════════════════════════════════════
function CategoryNav({ categories, activeCat, accent, onPick, beverageTheme }: { categories: any[]; activeCat: string; accent: string; onPick: (id: string) => void; beverageTheme?: boolean }) {
  if (categories.length <= 1) return null;
  return (
    <nav className="sticky top-[116px] md:top-[64px] z-30 backdrop-blur-xl mt-6" style={{ background: 'rgba(244,237,253,0.85)', borderTop: `1px solid ${CARD_BD}`, borderBottom: `1px solid ${CARD_BD}` }}>
          ...
          return (
            <button key={c.id} onClick={() => onPick(c.id)} className="shrink-0 flex items-center gap-2 pl-1.5 pr-4 h-11 rounded-full transition-all"
              ...
              <span className="w-8 h-8 rounded-full overflow-hidden flex items-center justify-center" style={{ background: on ? '#ffffff33' : LAV_SOFT }}>
                ...
                ) : beverageTheme ? <Sparkles className="h-4 w-4" style={{ color: on ? '#fff' : accent }} /> : <span className="text-[15px]">{categoryIcon(c.name)}</span>}
              </span>
...
//  SECTION HEAD
// ══════════════════════════════════════════════════════════════════════════════
function SectionHead({ title, icon, image, heart, accent, beverageTheme }: { title: string; icon?: string; image?: string | null; heart?: boolean; accent?: string; beverageTheme?: boolean }) {
  return (
    <div className="flex items-center gap-3 mb-5">
      {image ? (
        ...
        <span className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: LAV_SOFT, color: accent || '#8B5CF6' }}><Sparkles className="h-4 w-4" /></span>
      ) : null}
...
function ProductCard({ p, accent, onOpen, slug, beverageTheme, fallbackLogo }: { p: any; accent: string; onOpen: () => void; slug: string; beverageTheme?: boolean; fallbackLogo?: string | null }) {
  const fmt = useMoney();
  const lines = useCart(s => s.lines);
  const add = useCart(s => s.add);
  ...
  return (
    <article className={`group relative ${beverageTheme ? 'rounded-[20px] p-2.5' : 'rounded-[26px] p-3'} flex flex-col transition-all hover:-translate-y-1`} style={{ background: CREAM, border: `1px solid ${CARD_BD}`, boxShadow: beverageTheme ? '0 8px 20px rgba(83,55,119,.08)' : '0 10px 26px rgba(123,79,214,0.10)' }}>
      <button onClick={onOpen} className={`relative w-full ${beverageTheme ? 'aspect-[4/3] rounded-[16px]' : 'aspect-square rounded-[20px]'} overflow-hidden mb-3 block`} style={{ background: LAV_SOFT }}>
        ...
        ) : beverageTheme && fallbackLogo ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={cldImage(fallbackLogo, { width: 320 })} alt="" loading="lazy" decoding="async" className="h-full w-full object-contain p-5 opacity-70" />
      ...
      <div className="mt-auto pt-3 flex items-end justify-between gap-2">
        <div className="flex flex-col leading-none min-w-0">
          ...
          <span className="font-extrabold text-[18px]" style={{ color: beverageTheme ? accent : PINK, fontFamily: DISP }}>{fmt(pf.value)}</span>
        </div>
        {line && !needsModal(p) ? (
...
function DesktopCart({ accent, minOrder, allItems, onCheckout, onBrowse, onWhatsApp, waNumber, suggestions, onAddSuggestion, beverageTheme, logo }: any) {
  const fmt = useMoney();
  const { t } = useLang();
  const lines = useCart(s => s.lines);
  const total = useCart(s => s.total());
    ...
    <div className="rounded-[26px] overflow-hidden" style={{ background: CREAM, border: `1px solid ${CARD_BD}`, boxShadow: '0 16px 40px rgba(123,79,214,0.12)' }}>
      ...
      {lines.length === 0 ? (
        <div className="px-5 py-12 text-center">
          {beverageTheme && logo ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={cldImage(logo, { width: 180 })} alt="" className="mx-auto mb-3 h-20 w-20 object-contain opacity-70" />
...
function MobileCart({ accent, onClose, onCheckout, onWhatsApp, waNumber, allItems, minOrder, beverageTheme, logo }: any) {
  const fmt = useMoney();
  const { t } = useLang();
  const lines = useCart(s => s.lines);
  const total = useCart(s => s.total());
      ...
      <div onClick={e => e.stopPropagation()} className="relative w-full sm:max-w-md sm:ml-auto mt-auto sm:mt-0 h-[92vh] sm:h-full flex flex-col rounded-t-[28px] sm:rounded-none mk-slide" style={{ background: '#F7F1FD', borderLeft: `1px solid ${CARD_BD}` }}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {lines.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center">
              {beverageTheme && logo ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={cldImage(logo, { width: 220 })} alt="" className="mb-4 h-24 w-24 object-contain opacity-70" />
        ...
        {lines.length > 0 && (
          <div className="px-5 pt-4 pb-5 shrink-0 space-y-3" style={{ borderTop: `1px solid ${CARD_BD}`, background: CREAM }}>
            ...
            <button onClick={onCheckout} disabled={belowMin} className="w-full py-4 rounded-[18px] font-extrabold text-white active:scale-95 transition disabled:opacity-50 flex items-center justify-center gap-2" style={{ background: beverageTheme ? accent : PINK, fontFamily: DISP }}>{t('checkout')} <ChevronRight className="w-5 h-5" /></button>
    ...
    <div className="fixed inset-0 z-[120] flex items-end sm:items-center justify-center" onClick={onClose}>
      ...
      <div onClick={e => e.stopPropagation()} className="relative w-full sm:max-w-sm rounded-t-[28px] sm:rounded-[26px] p-6" style={{ background: '#F7F1FD', border: `1px solid ${CARD_BD}` }}>
        ...
        {mode === 'register' && (info.welcomeBonusPoints || 0) > 0 && (
          <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-2xl mb-3" style={{ background: `${accent}14`, border: `1px solid ${accent}55` }}>
            <Gift className="w-4 h-4 shrink-0" style={{ color: accent }} />
            <p className="text-[12px] font-extrabold leading-snug" style={{ color: accent }}>Crea tu cuenta y te regalamos {info.welcomeBonusPoints} puntos de bienvenida.</p>
          </div>
        )}
