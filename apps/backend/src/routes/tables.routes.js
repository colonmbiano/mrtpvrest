...
router.get('/', async (req, res) => {
  try {
    ...
    tables.sort((a, b) => {
      ...
      // Desempate por nombre de zona (dos zonas con el mismo `order`).
...
// impaciente inunde la caja tocando el botón repetidamente. Es best-effort
// por instancia — suficiente para el caso de uso (una sola caja por
// sucursal); no requiere Redis.
...
router.post('/:id/call', async (req, res) => {
  try {
    ...
    // Cooldown por mesa: descarta llamadas repetidas dentro de la ventana.
    const now = Date.now();
    const last = lastCallByTable.get(table.id) || 0;
    if (now - last < CALL_COOLDOWN_MS) {
      const retryInMs = CALL_COOLDOWN_MS - (now - last);
