...
export default function RetailAdminPage() {
  const [activeLocationId, setActiveLocationId] = useState(() =>
    typeof window !== "undefined" ? localStorage.getItem("moda-admin-locationId") || "" : "",
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!activeLocationId && locationRows[0]?.id) {
        localStorage.setItem("moda-admin-locationId",locationRows[0].id);
  ...
  useEffect(() => {
    const onStorage = (event: StorageEvent) => {
      if (event.key === "moda-admin-locationId") setActiveLocationId(event.newValue || "");
    };
    const onLocationChanged = () => setActiveLocationId(localStorage.getItem("moda-admin-locationId") || "");
  ...
  function selectLocation(nextLocationId: string) {
    localStorage.setItem("moda-admin-locationId",nextLocationId);
  ...
  return (
    <RetailShell>
      <RetailHeader
        ...
        subtitle={`${selectedLocation?.name || "Sucursal"} · productos, SKU, stock y traspasos`}
        actions={
          <>
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/80 p-4">
      <div className="tpv-retail my-4 w-full max-w-xl rounded-[2rem] border border-white/10 bg-[var(--surface-1)] p-5 shadow-2xl md:p-6">
