    ...
    <aside className="w-[420px] border-l border-bd bg-surf-1 flex flex-col shrink-0">
        ...
        <div className="flex-1 flex overflow-x-auto scrollbar-hide">
          {tickets.map((t, idx) => (
            <button 
              ...
              className={`px-6 h-full flex items-center justify-center text-[12px] font-bold border-r border-bd relative transition-colors ${
          ...
          <button 
            onClick={() => addTicket()}
            className="w-12 h-full flex items-center justify-center text-tx-mut hover:bg-surf-2 transition-colors"
          >
            <Plus size={18} />
          </button>
        </div>
      ...
      <div className="p-5 flex flex-col gap-4">
        <div className="flex justify-between items-center">
          <span className="eyebrow">ORDEN #{ticket.id} · BORRADOR</span>
          <span className="text-[11px] font-bold text-tx-mut mono tnum">
        ...
        <div className="flex gap-2">
          <div className="flex-1 bg-surf-2 border border-bd rounded-md h-[38px] flex items-center px-3 gap-2">
            ...
            <input 
              className="bg-transparent border-none outline-none text-[13px] font-semibold text-tx-pri w-full"
              value={ticket.name || "Publico General"}
              onChange={(e) => useTicketStore.getState().updateTicket({ name: e.target.value })}
            />
          </div>
          <Button variant="ghost" size="sm" className="w-[38px] h-[38px] p-0" onClick={clearActiveItems}>
      ...
      <div className="p-5 border-t border-bd bg-surf-2/50">
        <div className="flex flex-col gap-2 mb-5">
           ...
           <div className="flex justify-between items-center text-iris-500 pt-2 border-t border-bd/10">
             <span className="text-[13px] font-black uppercase tracking-widest">Total</span>
             <span className="text-3xl font-black mono tnum tracking-tighter">${total.toFixed(2)}</span>
           </div>
        </div>
