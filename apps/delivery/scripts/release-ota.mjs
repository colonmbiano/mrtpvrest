#!/usr/bin/env node
// Genera un bundle OTA de la app de delivery y lo publica en api.mrtpvrest.com.
//
// Uso:
//   pnpm --filter @mrtpvrest/delivery ota:release [--version 1.0.5] [--channel production] [--notes "..."]
//
...
//   4) POST multipart al backend → /api/ota/publish (appId=com.mrtpvrest.delivery)
//
...
const APP_ID = 'com.mrtpvrest.delivery';
...
function run(cmd, args, opts = {}) {
  console.log(`\n$ ${cmd} ${args.join(' ')}`);
  const r = execSync(`${cmd} ${args.join(' ')}`, { stdio: 'inherit', cwd: appRoot, ...opts });
  return r;
}
...
async function zipDir(srcDir) {
  ...
  try {
    ...
    throw new Error(
      'Falta la dep `archiver`. Instala con: pnpm --filter @mrtpvrest/delivery add -D archiver'
    );
  }
...
async function main() {
  ...
  console.log(`\n>>> Release OTA Delivery v${version} (channel=${args.channel})`);
  ...
  form.append('bundle', blob, `delivery-${version}.zip`);
