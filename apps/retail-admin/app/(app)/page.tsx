...
export default function DashboardPage() {
  ...
  const locationName =
    typeof window !== "undefined" ? localStorage.getItem("moda-loc-name") || "" : "";
  ...
  const stats = useMemo(() => {
    ...
    const salesToday = sales.filter(
      (s) => s.status === "COMPLETED" && s.createdAt?.slice(0, 10) === today,
    );
  ...
  return (
    <div className="mx-auto w-full max-w-[1200px]">
      <header className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          <div className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--brand-primary)]">
            MODA+ Admin
          </div>
          <h1
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        {/* Bajo mínimo */}
        <Card title="Stock bajo mínimo" href="/catalogo" cta="Ver stock">
          {loading ? (
            <Skeleton />
        ...
        {/* Ventas recientes */}
        <Card title="Ventas recientes" href="/catalogo" cta="Ver ventas">
          {loading ? (
            <Skeleton />
          ) : recent.length === 0 ? (
