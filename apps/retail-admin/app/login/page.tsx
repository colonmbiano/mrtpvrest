...
import type { RetailLocation } from "@/lib/auth";
...
export default function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // Elegir sucursal por defecto: la RETAIL si existe, si no la primera.
      try {
        const locRes = await api.get<RetailLocation[]>("/api/admin/locations");
        const locs = Array.isArray(locRes.data) ? locRes.data : [];
        const retail = locs.find((l) => l.businessType === "RETAIL");
        const chosen = retail || locs[0];
        if (chosen?.id) localStorage.setItem("locationId", chosen.id);
      } catch {
        /* sin sucursales todavía — el dashboard mostrará el estado vacío */
      }
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[var(--bg)] px-4 py-10 font-sans">
      <div className="w-full max-w-md">
        ...
        <p className="mt-5 text-center text-xs text-[var(--tx-dim)]">
          ¿Aún no tienes tienda? Créala desde la app <span className="font-bold text-[var(--tx-mut)]">MODA+</span> en tu
          teléfono o caja.
        </p>
      </div>
    </main>
