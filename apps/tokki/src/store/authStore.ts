...
import { setToken as vaultSetToken, getToken } from "@/lib/token-vault";
    ...
    (set, get) => ({
      ...
      loginWithPin: async (pin: string) => {
        ...
        try {
            ...
            const activateSession = (employee: TPVEmployee, token?: string) => {
              ...
              if (typeof window !== "undefined") {
                ...
                if (token) {
                  ...
                  // Un login offline nunca hereda el JWT del empleado anterior.
          ...
          if (isOnline) {
            try {
              const { data } = await api.post("/api/employees/login", { pin }, { timeout: LOGIN_TIMEOUT_MS });
              const token: string = data.token || data.accessToken;
              const employee: TPVEmployee = data.employee || data.user;
      ...
      hydrateFromStorage: async () => {
        ...
        if (!employee) return;
        if (!employee.isActive) { get().logout(); return; }
        try {
          const token = await getToken();
          if (!navigator.onLine) return;
          if (!token) { get().logout(); return; }
          const { data } = await api.get("/api/employees/me", { timeout: LOGIN_TIMEOUT_MS });
          if (get().employee?.id !== employee.id) return;
          if (!data.employee?.isActive || data.employee.id !== employee.id || data.location?.id !== localStorage.getItem("locationId")) {
            get().logout(); return;
          }
          set({ employee: data.employee, isAuthenticated: true });
        } catch (cause: unknown) {
          const status = (cause as { response?: { status?: number } }).response?.status;
          if (status && status >= 400 && status < 500 && get().employee?.id === employee.id) get().logout();
          // Una caída de red conserva el acceso offline; un rechazo no.
        }
      },
    }),
    {
      ...
      storage: createJSONStorage(() => ({
        ...
        async setItem(key, value) {
          try { await protectedStorage.setItem(key, value); }
          catch { if (typeof window !== "undefined") window.dispatchEvent(new Event("tokki-storage-error")); }
        },
      })),
