...
import { getToken, initTokenVault } from "@/lib/token-vault";
  ...
  async (error) => {
    if (typeof window !== "undefined" && error?.response?.status === 401) {
      ...
      if (!isLoginUrl && !isPublicRoute) {
        // Import diferido evita el ciclo api → authStore → api al arrancar.
        // Una respuesta vieja no debe cerrar la sesión de otro empleado.
        const sent = error.config?.headers?.Authorization;
        const current = await getToken();
        if (!current || sent === `Bearer ${current}`) {
          const { useAuthStore } = await import("@/store/authStore");
          useAuthStore.getState().logout();
          window.dispatchEvent(new Event("tokki-session-expired"));
        }
      }
    }
